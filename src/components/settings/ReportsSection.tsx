...
export const ReportsSection = ({ onBack }: ReportsSectionProps) => {
  const { transactions, categories, addNotification, projects, partners, userProfile, orgName, orgLogoUrl } = useFinanceStore();
  ...
  const reportOwnerName = organizationName || clientName;
  const sanitizeFilePart = (value: string) => value.replace(/[^a-zA-Z0-9\s-]/g, '').replace(/\s+/g, ' ').trim();
  const sanitizedReportOwnerName = sanitizeFilePart(reportOwnerName) || 'FinTrack Report';
  ...
  const handleExportCSV = () => {
    ...
    const rows = filteredTransactions.map(t => {
      ...
      const partner = findPartnerByHandledBy(partners, t.handledBy);
      return [
        t.date,
        t.time,
        (t.title || '').replace(/"/g, '""'),
    ...
    a.href = url;
    a.download = `${sanitizedReportOwnerName} - ${format(dateRange.start, 'MMM yyyy')} to ${format(dateRange.end, 'MMM yyyy')} - Report - FinTrack Plus.csv`;
    a.click();
  ...
  const handleExportCAPackage = async () => {
    ...
    try {
      ...
      const packageHeader = [
        'FinTrack+ - CA Export Package',
        organizationName ? `Organisation Name,"${organizationName.replace(/"/g, '""')}"` : '',
        `Client Name,"${clientName.replace(/"/g, '""')}"`,
        `Report Period,"${format(dateRange.start, 'MMM dd, yyyy')} to ${format(dateRange.end, 'MMM dd, yyyy')}"`,
        '',
      ].filter(Boolean);
      const transactionsCsv = [...packageHeader, csvHeaders.join(','), ...csvRows].join('\n');
...
`;
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `${sanitizedReportOwnerName} - CA Export - ${format(dateRange.start, 'MMM yyyy')} to ${format(dateRange.end, 'MMM yyyy')} - FinTrack Plus.zip`;
      a.click();
      URL.revokeObjectURL(url);
  ...
  const handleExportPDF = () => {
    ...
    const htmlContent = `
      ...
      <head>
        ...
        <title>${reportOwnerName} - Report - FinTrack Plus</title>
        <style>
          * { margin: 0; padding: 0; box-sizing: border-box; }
          ...
          .org-brand { display: inline-flex; align-items: center; justify-content: center; gap: 12px; margin-bottom: 8px; }
          .org-logo { width: 42px; height: 42px; object-fit: contain; border-radius: 10px; border: 1px solid #e5e7eb; }
          .org-name { color: #111827; font-size: 18px; font-weight: 800; }
      ...
      <body>
        <div class="header">
          ${organizationName || orgLogoUrl ? `
            <div class="org-brand">
              ${orgLogoUrl ? `<img class="org-logo" src="${orgLogoUrl}" alt="${organizationName || 'Organisation logo'}" />` : ''}
