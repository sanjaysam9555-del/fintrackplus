...
`;
  ...
  const handleExportPDF = () => {
    ...
    const transactionRows = filteredTransactions.map(t => {
      ...
      return `
        <tr>
          ...
          <td>${project?.name || '-'}</td>
          <td>${partner?.name || '-'}</td>
          <td style="text-align: right; font-weight: 500;">${formatCurrency(t.amount)}</td>
      ...
      <head>
        ...
        <style>
          * { margin: 0; padding: 0; box-sizing: border-box; }
          ...
          .header h1 { color: #6366f1; margin: 0 0 5px 0; font-size: 20px; }
          ...
          .meta-item { }
          .meta-label { color: #6b7280; font-size: 10px; text-transform: uppercase; margin-bottom: 3px; }
          .meta-value { font-weight: 600; font-size: 13px; word-break: break-word; }
          .summary { 
            display: flex; 
            flex-wrap: wrap;
            gap: 10px; 
    ...
    addNotification({
      ...
      message: `${filteredTransactions.length} transactions in report`,
    });
  };
      ...
      {createPortal(
        <AnimatePresence>
          {pdfPreviewHtml && (
            ...
            >
              {/* Top Bar */}
              <div className="shrink-0 flex items-center justify-between px-4 py-3 border-b border-border bg-card safe-top">
                <button
                  onClick={() => setPdfPreviewHtml(null)}
                  className="flex items-center gap-2 p-2 -ml-2 rounded-full hover:bg-muted"
                >
                  <ArrowLeft size={20} />
